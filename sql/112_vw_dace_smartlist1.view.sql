...
--           CASE m.DOCTYPE 
--				WHEN 6 THEN 
--					CASE WHEN MAX(CFDIA.METODOPAGO) = 'PUE' AND MAX(isnull(asoc.MexFolioFiscal, '')) = '' THEN 
--							MAX(APLICADO.MexFolioFiscal) 
--						ELSE MAX(asoc.MexFolioFiscal) 
--					END 
--				ELSE 
--					MAX(asoc.MexFolioFiscal) 
--			END AS CFDI_ASOC, 
--			CASE m.DOCTYPE 
--				WHEN 1 THEN 
--					CASE WHEN (CFDI.METODOPAGO) = 'PUE' THEN 
--							MAX(asoc.MexFolioFiscal) 
--						ELSE MAX(aplicado.MexFolioFiscal) 
--					END 
--				WHEN 6 THEN 
--					CASE WHEN (CFDIa.METODOPAGO) = 'PUE' THEN 
--							MAX(aplicado.MexFolioFiscal) 
--						ELSE MAX(aplicado.MexFolioFiscal) 
--					END 
--			END AS CFDI_Apli, 
--			ISNULL(CFDI.METODOPAGO, '') AS MetodoPago, 
--            CFDIA.METODOPAGO AS cfdia_metodopago, 
--			CASE WHEN CFDI.METODOPAGO = 'PUE' OR CFDIA.METODOPAGO = 'PUE' THEN 
--					CASE WHEN m.DOCTYPE = 1 AND ISNULL(MAX(aplicado.MexFolioFiscal), '') = '' AND MAX(a.DocAplic) IS NOT NULL THEN 
--							'CFDI SE APLICO OK' 
--						ELSE 
--							CASE WHEN m.DOCTYPE = 6 AND ISNULL(MAX(asoc.MexFolioFiscal), '') = '' THEN 
--								'CFDI SE APLICO OK' 
--								ELSE 'CFDI SE APLICO ERRADO' 
--							END 
--					END 
--				ELSE 
--					CASE WHEN MIN(dbo.fun_DACE_ValidarAplica(
--													CASE m.DOCTYPE WHEN 1 THEN aplicado.MexFolioFiscal
--																	WHEN 6 THEN asoc.MexFolioFiscal 
--													END, 
--													CASE m.DOCTYPE WHEN 1 THEN asoc.MexFolioFiscal 
--																	WHEN 6 THEN aplicado.MexFolioFiscal END)) > 0 THEN 
--							'CFDI SE APLICO OK' 
--						ELSE 'CFDI SE APLICO ERRADO' 
--					END 
--			END AS OBSERVACIONES, 
--			m.DCSTATUS, pm.TXRGNNUM, pm.VENDNAME
